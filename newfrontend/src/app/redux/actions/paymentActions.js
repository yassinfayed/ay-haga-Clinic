...
export const makeOrder = (body, fm) => async (dispatch) => {
  ...
  try {
    ...
    if (body.paymentMethod === "Stripe") {
      let url =
        body.reserve === true
          ? `${baseURL}/api/v1/appointment/reserveStripe/${body.doctor}/${body.price}`
          : `${baseURL}/api/v1/healthPackages/subscribeStripe/${body.id}`;
      if (fm) {
        url += `?fm=${fm}`;
      }
      ...
      let url =
        body.reserve === true
          ? `${baseURL}/api/v1/appointment/reserve/${body.doctor}/${body.price}`
          : `${baseURL}/api/v1/healthPackages/subscribe/${body.id}`;
      if (fm) {
        url += `?fm=${fm}`;
      }
      if (body.reserve && fm) url += `&date=${body.date}`;
