...
exports.acceptEmploymentContract = catchAsync(async (req, res, next) => {
  const doctor = await Doctor.findOne({ user: req.user._id });
  const doctorID = doctor._id;
  const doctorfound = await Doctor.findByIdAndUpdate(
    doctorID,
    {
      employmentContract: {
...
exports.addAvailableDate = catchAsync(async (req, res, next) => {
  ...
  if (!req.body.availableDate) res.status(200).json({});
  //what if the same date entered twice

  const doctorfound = await Doctor.findByIdAndUpdate(
    doctorID,
    { availableDates: [...oldAvailableDates, req.body.availableDate] },
    {
      new: true,
      runValidators: true,
    }
...
exports.acceptDoctor = catchAsync(async (req, res, next) => {
  ...
  const doctor = await Doctor.findByIdAndUpdate(
    req.params.id,
    {
      isApproved: true,
      employmentContract: {
        hourlyRate: doctorfound.HourlyRate,
        status: "Pending",
      },
    },
    {
